...
* I made a tree with 153 taxa on it, and most of the names have been changed to match my data (data.2004.pic). But there are some things to fix:
...
setdiff(data.2004.pic$taxon.new, data.2004.pic.noNA$taxon.new)
...
matched <- intersect(data.2004.pic.noNA$taxon.new, megaphylogeny$tip.label) # 71 matches
unmatched <- setdiff(data.2004.pic.noNA$taxon.new, megaphylogeny$tip.label) # 71 non-matches
```
...
# Look at species that used fuzzy matching algorithm - 5 rows
...
matched.syn <- union(matched, intersect(unmatchedTPL.noNA.syns$Accepted_SPNAME2, megaphylogeny$tip.label)) # 88 matches (added 17 to the original 71 matches)

# Update list of TPL names of spp STILL not on megaphylogeny. 
unmatched.syn <- setdiff(unmatchedTPL.noNA$Accepted_SPNAME2, megaphylogeny$tip.label) # 53
```

I moved this here for now so I can get the number of species still not included after using the The Plant List to find synonyms.
```{r}
data.2004.pic %<>% dplyr::mutate(taxon.new = ifelse(taxon.new == "Opuntia_macrocenta", "Opuntia_macrocentra", taxon.new),
                                 taxon.new = ifelse(taxon.new == "Lesquerella_fendleri", "Physaria_fendleri", taxon.new),
                                 taxon.new = ifelse(taxon.new == "Ecinomastus_intertextus", "Echinocactus_intertextus", taxon.new),
...
unmatchedTPL.unmatched.syn <- unmatchedTPL.noNA[unmatchedTPL.noNA$Accepted_SPNAME2 %in% unmatched.syn, ]  # 53 rows

# Extract TPL genus names for unmatched.syn vector
unmatched.syn.genera <- unmatchedTPL.unmatched.syn$New.Genus   # 53

# Vector of TPL genera in unmatched.syn that are NOT present in Zanne tree
unmatched.syn.noTree.genera <- setdiff(unmatched.syn.genera, megaphylogeny_genera)  # 42

# Subset unmatched TPL to rows of genera not on Zanne tree (53 rows, 41 genera)
...
unmatched.orig.names <- unmatchedTPL.unmatched.syn.noTree.genera$Submitted_Name2 %>% sort()   # 53 
unmatched.orig.genera <- unmatchedTPL.unmatched.syn.noTree.genera$Submitted_Genus %>% unique() %>% sort() # 41

# Combined list of unmatched synonyms and original spp NOT in Zanne tree. 62 species (many are redundant but a few aren't - see setdiff lines below for non-redundant taxa)
unmatched.all <- union(unmatched.syn, unmatched.orig.names) %>% sort()  # 62
...
length(pruned.tree$tip.label) # 141
...
I need to rename the tree tips that were replaced with synonyms, so that they match Kamren's data names (data.2004.pic.noNA$taxon.new)
```{r}
...
ordered.tips <- pruned.tree$tip.label %>% sort() # 150

# Original names in alphabetical order
ordered.taxa <- data.2004.pic.noNA$taxon.new %>% unique() %>% sort() # 150
...
# Make a key for replacements. "Old_name" = "New_name" (or "temp.tree$tip.label" = "data.2004.noNA$taxon.new")
...
setdiff(temp.tree$tip.label, data.2004.pic.noNA$taxon.new)
...
setdiff(data.2004.pic.noNA$taxon.new, temp.tree$tip.label)

# Check database results for synonyms of these species
unmatchedTPL.noNA %>%
  dplyr::filter(Submitted_Name2 %in% setdiff(data.2004.pic.noNA$taxon.new, temp.tree$tip.label)) %>%
...
data.2004.pic.noNA %>%
  filter(taxon.new %in% setdiff(temp.tree$tip.label, data.2004.pic.noNA$taxon.new))
```
...
```{r}
SpToRemove <- setdiff(data.2004.pic.noNA$taxon.new, temp.tree$tip.label)
SpToRemove

new.pic.data <- data.2004.pic.noNA %>% filter(!taxon.new %in% SpToRemove)
```
...
```{r}
temp.tree <- drop.tip(temp.tree, setdiff(temp.tree$tip.label, data.2004.pic.noNA$taxon.new))
length(temp.tree$tip.label)
```
...
```{r}
#write tree files
write.tree(temp.tree, file="~/GitHub/SEV_Species_Interactions_Stability/data/phylo_tree/Kamren.2004.tre") 

#Newick format
write.nexus(temp.tree, file="~/GitHub/SEV_Species_Interactions_Stability/data/phylo_tree/Kamren.2004.nex") #Nexus format
...
data.2004.pic.noNA$taxon.new %>% unique() %>% sort()
