...
matched <- intersect(data.2004.pic.noNA$taxon.new, megaphylogeny$tip.label) # 76 matches
unmatched <- setdiff(data.2004.pic.noNA$taxon.new, megaphylogeny$tip.label) # 78 non-matches
```
...
matched.syn <- union(matched, intersect(unmatchedTPL.noNA.syns$Accepted_SPNAME2, megaphylogeny$tip.label)) # 94 matches (added 17 to the original 76 matches)
...
unmatched.syn <- setdiff(unmatchedTPL.noNA$Accepted_SPNAME2, megaphylogeny$tip.label) # 56
```
...
unmatched.all <- union(unmatched.syn, unmatched.orig.names) %>% sort()  # 63
...
length(pruned.tree$tip.label) # 150
