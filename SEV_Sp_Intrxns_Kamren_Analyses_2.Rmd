...
* I'm on line 667 - trying to decide how to deal with the fact that for the bird data, they went to each transect 3 times, whereas before that they only went once. Gonna talk to Jenn about it.
* Bird data only have 11 years, while bee and mammal data have 17. So I should probably also analyze bee and mammal data from 1991 to 2025.
...
# Load libraries
libraries <- c("magrittr", "lme4", "car", "emmeans", "MuMIn", "scales", "sjPlot", "vegan", "ape", "phytools", "devtools", "ggbiplot", "tidyverse", "googlesheets4", "brms", "reshape2", "DHARMa")
...
data.species <- read.csv("~/GitHub/SEV_Species_Interactions_Stability/data/SevilletaSpeciesList2019.csv", stringsAsFactors = T) # Read in species data to get native/invasive status
...
# Fix LifeHistory 
levels(data.intrxns$LifeHistory)
...
# Join in data for native/invasive status, merge datasets
data.species %<>% dplyr::select(kartez, Native)
data <- left_join(data.intrxns, data.species, by = "kartez")
data <- left_join(data, mass_species_cast, by = "kartez")
...
# NOMI is the only species with "(likely W)A" dispersal level. Let's call it WA for now.
data %>% filter(Dispersal == "(likely W)A") %>% group_by(kartez) %>% summarise(n = n())
data %<>% mutate(Dispersal = fct_recode(Dispersal, "WA" = "(likely W)A"))
...
# Filter out "Sporobolus_NA", "Opuntia_NA", "Astragalus_NA", and "Aristida_NA". These are kartez: ASTRA, ARIST, OPUNT, SPORO
data %<>% filter(!kartez %in% c("SPORO", "OPUNT", "ASTRA", "ARIST")) 
...
# Change character and some numeric vectors to factors
data.mass %<>% mutate_if(is.character, as.factor)
data.mass$web <- as.factor(data.mass$web)
quad.data %<>% mutate_if(is.character, as.factor)
...
```{r}
# List of dfs
#dfs.list <- list(data.2004, data.2006, data.2008, data.2011, data.2012)
dfs.list <- list(data.2004, data.2012)

# List of df names
#dfs.list.names <- list("data.2004", "data.2006", "data.2008", "data.2011", "data.2012")
dfs.list.names <- list("data.2004", "data.2012")

# List of end column numbers (final year of dataset)
#end.cols <- list(54, 52, 50, 47, 46)
end.cols <- list(55, 47)
...
2012: Simplified categories for pollination and dispersal
...
```{r, message=F}
### Full model
mod.all.2012.f2.gps <- lmer(species_CV ~ Pollination.new + Dispersal.new + Legume + X + Y + (1|kartez), na.action = na.omit, data = data.2012 %>% filter(kartez != "CHER2"))
Anova(mod.all.2012.f2.gps, type = 2)
plot(mod.all.2012.f2.gps)

### Separate model for each predictor
mod.pol.2012.f2.gps <- lmer(species_CV ~ Pollination.new + X + Y + (1|kartez), na.action = na.omit, data = data.2012 %>% filter(kartez != "CHER2")) # Filter out the species that we only have family-level data for
mod.disp.2012.f2.gps <- lmer(species_CV ~ Dispersal.new + X + Y + (1|kartez), na.action = na.omit, data = data.2012)
mod.leg.2012.f2.gps <- lmer(species_CV ~ Legume + X + Y + (1|kartez), na.action = na.omit, data = data.2012)

Anova(mod.pol.2012.f2.gps, type = 2)
Anova(mod.disp.2012.f2.gps, type = 2)
Anova(mod.leg.2012.f2.gps, type = 2)

### Effect sizes (% increase for animal-pollinated or -dispersed)

# Pollination
pol.emm <- summary(emmeans(mod.pol.2012.f2.gps, pairwise ~ Pollination.new))
(pol.emm$emmeans[1, 2] - pol.emm$emmeans[2, 2]) / pol.emm$emmeans[2, 2] * 100

# Dispersal
disp.emm <- summary(emmeans(mod.disp.2012.f2.gps, pairwise ~ Dispersal.new))
...
# Get predictions
library(ggeffects)
pred_pol <- ggpredict(mod.pol.2012.f2.gps, terms = "Pollination.new")
pred_pol
...
# Get predictions
library(ggeffects)
pred_disp <- ggpredict(mod.disp.2012.f2.gps, terms = "Dispersal.new")
pred_disp
...
# How many perennial wind-pollinated species are grasses?
data.2012 %>%
...
```{r}
### Separate model for each predictor ###
mod.pol.2012.f3.gps.mu <- lmer(species_CV ~ Pollination.new * species_mu + X + Y + (1|kartez), na.action = na.omit, data = data.2012 %>% filter(Pollination != "WA"))
mod.disp.2012.f3.gps.mu <- lmer(species_CV ~ Dispersal.new * species_mu + X + Y + (1|kartez), na.action = na.omit, data = data.2012 %>% filter(Dispersal != "WA"))
mod.leg.2012.f3.gps.mu <- lmer(species_CV ~ Legume * species_mu + X + Y + (1|kartez), na.action = na.omit, data = data.2012)

Anova(mod.pol.2012.f3.gps.mu, type = 2)
Anova(mod.disp.2012.f3.gps.mu, type = 2)
Anova(mod.leg.2012.f3.gps.mu, type = 2)

# Plot marginal effect of each predictor
Pol.mu.plot <- plot_model(mod.pol.2012.f3.gps.mu, type = "pred", terms = "Pollination.new", color = "black") +
  scale_x_discrete(limits = c("A", "W"),
                   labels = c("Animal", "Wind")) +
  labs(y = "CV of Species Biomass", x = "Pollination Strategy") +
  ylim(c(1.7, 2.9))
...
Disp.mu.plot <- plot_model(mod.disp.2012.f3.gps.mu, type = "pred", terms = "Dispersal.new", color = "black")  +
  scale_x_discrete(limits = c("A", "W"),
                   labels = c("Animal", "Wind")) +
...
### Full model ###
mod.all.2012.f3.gps <- lmer(species_CV ~ Pollination + Dispersal + Legume + X + Y + (1|kartez), na.action = na.omit, data = data.2012 %>% filter(Pollination != "WA" & Dispersal != "WA"))
Anova(mod.all.2012.f3.gps, type = 2)
plot(mod.all.2012.f3.gps)

### Separate model for each predictor ###
mod.pol.2012.f3.gps <- lmer(species_CV ~ Pollination + X + Y + (1|kartez), na.action = na.omit, data = data.2012 %>% filter(Pollination != "WA"))
mod.disp.2012.f3.gps <- lmer(species_CV ~ Dispersal + X + Y + (1|kartez), na.action = na.omit, data = data.2012 %>% filter(Dispersal != "WA"))
mod.leg.2012.f3.gps <- lmer(species_CV ~ Legume + X + Y + (1|kartez), na.action = na.omit, data = data.2012)

Anova(mod.pol.2012.f3.gps, type = 2)
Anova(mod.disp.2012.f3.gps, type = 2)
Anova(mod.leg.2012.f3.gps, type = 2)

# Plot marginal effect of each predictor
plot_model(mod.pol.2012.f3.gps, type = "pred", terms = "Pollination")
plot_model(mod.disp.2012.f3.gps, type = "pred", terms = "Dispersal")
plot_model(mod.leg.2012.f3.gps, type = "pred", terms = "Legume")


# Plot marginal effect of each predictor
Pol.plot <- plot_model(mod.pol.2012.f3.gps, type = "pred", terms = "Pollination") +
  scale_x_discrete(limits = c("A", "W"),
                   labels = c("Animal", "Wind")) +
  labs(y = "CV of Species Biomass", x = "Pollination Strategy") +
  ylim(c(2.2, 3))
...
Disp.plot <- plot_model(mod.disp.2012.f3.gps, type = "pred", terms = "Dispersal")  +
  scale_x_discrete(limits = c("A", "W"),
                   labels = c("Animal", "Wind")) +
  labs(y = "CV of Species Biomass", x = "Dispersal Strategy") +
  ylim(c(2.2, 3))
...
Leg.plot <- plot_model(mod.leg.2012.f3.gps, type = "pred", terms = "Legume")  +
...
### Fit model with Pollination x Dispersal
mod.poldiso.2012.f3.gps <- lmer(species_CV ~ Pollination * Dispersal + X + Y + (1|kartez), na.action = na.omit, data = data.2012 %>% filter(Pollination != "WA" & Dispersal != "WA"))
Anova(mod.poldiso.2012.f3.gps, type = 2)
...
```{r, eval=F}
### Separate model for each predictor ###
mod.pol.2012.f3.gps.mu <- lmer(species_CV ~ Pollination * species_mu + X + Y + (1|kartez), na.action = na.omit, data = data.2012 %>% filter(Pollination != "WA"))
mod.disp.2012.f3.gps.mu <- lmer(species_CV ~ Dispersal * species_mu + X + Y + (1|kartez), na.action = na.omit, data = data.2012 %>% filter(Dispersal != "WA"))
mod.leg.2012.f3.gps.mu <- lmer(species_CV ~ Legume * species_mu + X + Y + (1|kartez), na.action = na.omit, data = data.2012)
...
# Plot marginal effect of each predictor
Pol.mu.plot <- plot_model(mod.pol.2012.f3.gps.mu, type = "pred", terms = "Pollination") +
  scale_x_discrete(limits = c("A", "W"),
                   labels = c("Animal", "Wind")) +
  labs(y = "CV of Species Biomass", x = "Pollination Strategy") +
  ylim(c(1.6, 2.8))
...
Disp.mu.plot <- plot_model(mod.disp.2012.f3.gps.mu, type = "pred", terms = "Dispersal")  +
  scale_x_discrete(limits = c("A", "W"),
                   labels = c("Animal", "Wind")) +
  labs(y = "CV of Species Biomass", x = "Dispersal Strategy") +
  ylim(c(1.6, 2.8))
